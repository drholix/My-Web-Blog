...
Repositori ini menyiapkan situs statis **Hugo + layout responsif kustom** dengan konten dwibahasa (Indonesia & Inggris), siap deploy di **Netlify** dan diamankan oleh **Cloudflare**. Seluruh panduan di bawah berfungsi sebagai _runbook_ QA + DevOps: mulai dari setup lokal, konfigurasi inti, alur konten, hingga checklist produksi.
...
| --- | --- | --- |
| Halaman kosong saat build | Template dasar tidak ditemukan | Pastikan `layouts/_default/*.html` tersedia atau jalankan ulang build setelah commit terbaru. |
...
- Tema kustom: stylesheet internal (`assets/css/site.css`).
